...
import { toUsDate } from '../../core/dateUtils.js';

const DAYS_IN_WEEK = ["CN", "T2", "T3", "T4", "T5", "T6", "T7"];

export default function ComboScheduleManager({
  ...
  const addMakeUpSession = () => {
    const newSessions = [...selectedDates];
    newSessions.push({
      id: Math.random().toString(36).substring(7),
      date: toUsDate(new Date()),
      isTrial: false,
      status: "PENDING",
      isMakeUp: true,
    });
    ...
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm w-full overflow-hidden flex flex-col md:flex-row">
      ...
      <div className="flex-1 bg-gray-50 flex flex-col h-[400px]">
        <div className="p-3 border-b border-gray-200 bg-white flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={addMakeUpSession}
            className="flex items-center gap-1 px-2 py-1 bg-white border border-gray-200 rounded text-xs font-medium text-gray-700 hover:bg-gray-50"
          >
            <Plus className="w-3 h-3" /> Lịch bù
          </button>
        </div>
                ...
                <div key={session.id} className="bg-white border border-gray-200 rounded-lg p-2 shadow-sm flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold text-white ${session.isTrial ? 'bg-emerald-500' : session.isMakeUp ? 'bg-amber-500' : 'bg-pink-500'}`}>
                        {session.isTrial ? 'Trải nghiệm' : session.isMakeUp ? `Lịch bù (B.${normalIdx})` : `Buổi ${normalIdx}`}
                      </span>
                      <input 
                        type="date" 
                        value={session.date}
                        onChange={(e) => updateSession(session.id, { date: e.target.value })}
                        className="text-xs font-semibold text-gray-800 border-none bg-transparent p-0 focus:ring-0 cursor-pointer hover:text-indigo-600"
                      />
                    </div>
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 p-1.5 rounded-md">
                    ...
                    <select
                      ...
                      className={`text-[11px] font-medium rounded border-gray-200 py-0.5 pl-2 pr-6 ${
                        session.status === 'ATTENDED' ? 'text-blue-600 bg-blue-50' :
                        session.status === 'NO_SHOW' ? 'text-red-600 bg-red-50' :
